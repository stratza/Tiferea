...
[![Version](https://img.shields.io/badge/version-0.3.0-6e6e6e?style=for-the-badge&labelColor=1a1a1a)](CHANGELOG.md)
...
```sh
kubectl apply -f https://github.com/stratza/tiferea/releases/latest/download/tifera-0.3.0.yaml
# …or Helm:  helm install tifera oci://ghcr.io/stratza/charts/tifera --version 0.3.0 -n tifera --create-namespace
...
```sh
docker build -t tifera:0.3.0 backend
kind load docker-image tifera:0.3.0   # or: k3d image import tifera:0.3.0
...
1. **Build** on a connected machine: `docker build -t tifera:0.3.0 backend` (add `--build-arg KUBECTL_VERSION=v1.31.4` to pin kubectl; `docker buildx --platform ...` for other arches).
...
4. **Deploy** with the image rewritten: `sed 's#image: tifera:0.3.0#image: registry.internal/tifera:0.3.0#' deploy/tifera.yaml | kubectl apply -f -`, or Helm `--set image.repository=…,image.tag=0.3.0`.
</details>
